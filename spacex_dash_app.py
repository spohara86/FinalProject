...
site_list=pd.DataFrame(np.append(spacex_df['Launch Site'].unique(),['ALL']))
...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site-dropdown',
                                            options=[{'label': i, 'value': i} for i in site_list[0]],
...
def get_scatter_chart(entered_site,payload_range):
    ...
    filtered_df = payload_df[spacex_df['Launch Site']==entered_site]
    if entered_site == 'ALL':
        fig = px.scatter(payload_df,  x='Payload Mass (kg)', y='class', 
            color='Booster Version',title='Launch Success')
        return fig
    else:
